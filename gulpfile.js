...
var reportError = function (error) {
  var lineNumber = (error.cause) ? 'LINE ' + (error.lineNumber || error.cause.line) : ((error.lineNumber) ? 'LINE ' + error.lineNumber : '');
  ...
  if (error.fileName)   { report += errorChalk('FILE:') + ' ' + error.fileName + '\n'; }
  if (lineNumber) { report += errorChalk('LINE:') + ' ' + lineNumber + '\n'; }
...
gulp.task('convertfont', ['convertFonts']);

// Serve application:
var serveTasks = ['sass', 'copyCss', 'html', 'copyScripts', 'fonts', 'assets', 'images'];
gulp.task('serve', serveTasks, onServeTaskInit);

console.log(frameworkName + chalk.yellow(' DIGIFAST FRAMEWORK is setting up. Please wait a moment!'));
...
gulp.task('default', serveTasks, watchTask);
...
function watchSamsungDir() {
  ...
  mainJsContent = mainJsContent.substring(0, mainJsContent.indexOf("var GLoader"));
  ...
  function insertScriptsIntoHtml(htmlFile){
    ...
    var scriptTags = getAllStringsFromTo(contentOfBody, "<script src=", "</script>", true);
    ...
    for(i=0; i<files.length; i++){
      ...
      if(fileSrcType == "css"){
        // remove "../":
        fileContent = fileContent.replace(new RegExp("../fonts", 'g'), "fonts");
        contentOfBody += "\n  <style>";
      } else {
    ...
    if(shouldCallJsInit){
      ...
      if(idOfMainTag){
        ...
        var mainJsContent = fs.readFileSync("debug/js/pages/" + idOfMainTag + ".js", 'utf8');
        contentOfBody += "\n  " + mainJsContent;
        contentOfBody += "\n  " + idOfMainTag + ".init();";
        contentOfBody += "\n  </script>";
      }
  ...
  function searchForFiles(type){
    ...
    if(index > -1){
      var endIndex = mainJsContent.indexOf("." + type.toLowerCase() + "\"", index + 1);
      var fileUrl = mainJsContent.substring(index + 1, endIndex + type.length + 1);
...
function getAllStringsFromTo(content, fromStr, toStr, isIncludeStartAndEnd){
  ...
  for(var i=0; i<t; i++){
    var indexOfFromStr = (isIncludeStartAndEnd) ? content.indexOf(fromStr, si) : (content.indexOf(fromStr, si) + fromStr.length);
    var indexOfToStr = (isIncludeStartAndEnd) ? (content.indexOf(toStr, indexOfFromStr) + toStr.length) : content.indexOf(toStr, indexOfFromStr);
...
function getStringFromTo(content, fromStr, toStr, isIncludeStartAndEnd){
  if(!isIncludeStartAndEnd) isIncludeStartAndEnd = false;
  var indexOfFromStr = (isIncludeStartAndEnd) ? content.indexOf(fromStr) : (content.indexOf(fromStr) + fromStr.length);
  var indexOfToStr = (isIncludeStartAndEnd) ? (content.indexOf(toStr, indexOfFromStr) + toStr.length) : content.indexOf(toStr, indexOfFromStr);
...
function startWatching(dir, callback){
  ...
  var watcher0 = chokidar.watch('src/css/');

  var watcher1 = gulp.watch('src/*.html', ['html']);
  var watcher2 = chokidar.watch('src/common/');
  var watcher3 = chokidar.watch('src/assets/');
  var watcher4 = chokidar.watch('src/js/');
  var watcher5 = chokidar.watch('src/images/');

  var syncHtmlInt;
  watcher1.on('change', onSyncHTML);
  ...
  function checkScanningComplete(){
    ...
    if(countScannedFolder == totalScannedFolder){
      ...
      console.log(frameworkName + chalk.yellow(' DIGIFAST FRAMEWORK IS READY NOW. Happy Coding, Buddy!'));
  ...
  function onChokRaw(event, path, details){
    if(path.indexOf(".DS_Store") < 0){
      ...
      console.log("RAW:", event, details.path + " [ID: " + details.flags + "]");
    }
  }
  ...
  function onChokChange(event, eventPath){
    ...
    if(eventPath.indexOf(".DS_Store") < 0){ // ko cần execute nếu là file này
  ...
  function minifyAndExportCSS(srcFilePath, debugDirPath, buildDirPath, pathToDebug, pathToBuild){
    ...
    function writeLog(){
      ...
      console.log(frameworkName + " Minified CSS: '" + fileName + "' - Saved: " + savedStr + " (" + chalk.hex('#999999')(oldSize + " kB") + " -> " + newSize + " kB)");

      browserSync.reload();
    }
  }

  function minifyAndExportJS(srcFilePath, debugDirPath, buildDirPath, pathToDebug, pathToBuild){
    ...
    function writeLog(){
      ...
      var fileName = chalk.blue( paths[paths.length-1] );
      var savedStr = chalk.green("-" + optimizedRate + "%");
      console.log(frameworkName + " Minified JS: '" + fileName + "' - Saved: " + savedStr + " (" + chalk.hex('#999999')(oldSize + " kB") + " -> " + newSize + " kB)");  
  ...
  function minifyAndExportImage(srcFilePath, debugDirPath, buildDirPath, pathToDebug, pathToBuild){
  ...
  function copyFileAndMoveTo(srcFilePath, debugDirPath, buildDirPath){
    ...
    console.log(frameworkName + " Copying file '" + chalk.blue(srcFilePath) + "' to DEBUG...");
    ...
    .on('error', reportError)
    .on('end', function(){
      console.log(frameworkName + " File '" + chalk.blue(srcFilePath) + "' was copied to DEBUG successfully!");
      browserSync.reload();
    });
  }

  function onSyncHTML(event){
